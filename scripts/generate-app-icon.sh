...
help_info="$PWD/Free Ruler/FreeRuler.help/Contents/Info.plist"
legacy_help_icon="$help_shared_dir/freeruler.png"
...
build_number="$(/usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' "$help_info")"
build_token="$(printf '%s' "$build_number" | tr -cd '[:alnum:]_.-')"
help_icon_name="freeruler-help-icon-build-${build_token}-${icon_hash}.png"
help_icon="$help_shared_dir/$help_icon_name"

cp "$output_dir/icon_512x512.png" "$help_icon"
cp "$output_dir/icon_512x512.png" "$legacy_help_icon"
...
HELP_CACHE_TOKEN="build-${build_token}-${icon_hash}" perl -0pi -e \
